...
GEMINI_KEY = os.getenv("GEMINI_KEY_6")
DATA_DIR = Path("/mnt/d/projects/prompt-wall/agent-strike/data")
INPUT_FILE = DATA_DIR / "final_training_dataset.csv"
VERIFIED_FILE = DATA_DIR / "verified_k6.csv"
DISPUTED_FILE = DATA_DIR / "disputed_k6.csv"
CHECKPOINT_FILE = DATA_DIR / "checkpoint_k6.json"
BATCH_START = 745
BATCH_END = 891
BATCH_SIZE = 50
SLEEP = 5
...
def call_flash(rows):
    ...
    for attempt in range(5):
        try:
            ...
            err = str(e)
            if "429" in err:
                print(f"[K6] 429 — wait {RETRY_429}s (attempt {attempt+1}/5)")
                time.sleep(RETRY_429)
            elif "503" in err:
                print(f"[K6] 503 — wait {RETRY_503}s (attempt {attempt+1}/5)")
...
def process():
    ...
    print(f"[K6] Loaded {len(df)} rows.")
    checkpoint = load_checkpoint()
    start = checkpoint["last_completed_batch"] + 1
    print(f"[K6] Resuming from batch {start}.")
    ...
    for batch_idx in range(start, BATCH_END + 1):
        ...
        print(f"[K6] Batch {batch_idx+1}/892 — rows {batch.index[0]}–{batch.index[-1]}", flush=True)

        verdicts = call_flash(rows)
        if verdicts is None:
            print(f"[K6] Failed batch {batch_idx+1}. Skipping.")
        ...
        if (batch_idx + 1) % 10 == 0:
            ...
            verified, disputed = [], []
            print(f"[K6] Flushed at batch {batch_idx+1}.", flush=True)
        time.sleep(SLEEP)
