...
GEMINI_KEY = os.getenv("GEMINI_KEY_2")
DATA_DIR = Path("/mnt/d/projects/prompt-wall/agent-strike/data")
INPUT_FILE = DATA_DIR / "final_training_dataset.csv"
VERIFIED_FILE = DATA_DIR / "verified_k2.csv"
DISPUTED_FILE = DATA_DIR / "disputed_k2.csv"
CHECKPOINT_FILE = DATA_DIR / "checkpoint_k2.json"
BATCH_START = 149
BATCH_END = 297
BATCH_SIZE = 50
SLEEP = 5
...
def call_flash(rows):
    ...
    for attempt in range(5):
        try:
            ...
            err = str(e)
            if "429" in err:
                print(f"[K2] 429 — wait {RETRY_429}s (attempt {attempt+1}/5)")
                time.sleep(RETRY_429)
            elif "503" in err:
                print(f"[K2] 503 — wait {RETRY_503}s (attempt {attempt+1}/5)")
...
def process():
    ...
    print(f"[K2] Loaded {len(df)} rows.")
    checkpoint = load_checkpoint()
    start = checkpoint["last_completed_batch"] + 1
    print(f"[K2] Resuming from batch {start}.")
    ...
    for batch_idx in range(start, BATCH_END + 1):
        ...
        print(f"[K2] Batch {batch_idx+1}/892 — rows {batch.index[0]}–{batch.index[-1]}", flush=True)

        verdicts = call_flash(rows)
        if verdicts is None:
            print(f"[K2] Failed batch {batch_idx+1}. Skipping.")
        ...
        if (batch_idx + 1) % 10 == 0:
            ...
            verified, disputed = [], []
            print(f"[K2] Flushed at batch {batch_idx+1}.", flush=True)
        time.sleep(SLEEP)
